...
const CartSideBar = ({ userId }) => {
  ...
  return (
    <div className={`fixed top-16 right-0 h-full transition-transform duration-300 ${isOpen ? "translate-x-0" : "translate-x-[90%]"}`}>
      <div className={`relative h-full shadow-lg p-4 w-96 ${isOpen ? "bg-violet-100" : "bg-violet-300"}`}>
        <button
          onClick={toggleCart}
          className={`hover:!border-transparent focus:outline-none
            absolute top-1/2 transform -translate-y-1/2 -left-8 p-8 
            ${isOpen ? "bg-violet-100  text-gray-600" : "bg-violet-300 text-white"} rounded-full 
        ...
        <div className={`transform -rotate-90 -translate-x-1/2 ${isOpen ? "invisible" : "visible"}`}>
            {status === "loading" ? (
              ...
              <>
                ...
                <span className="text-xl font-semibold text-white">${animatedTotal.toFixed(2)}</span>
              </>
            ) : (
        ...
        <div className="pl-8">
          ...
          {cart && cart.items.length > 0 ? (
            <div>
              {cart.items.map((item, index) => (
                <div key={index} className="flex justify-between py-2 border-b transition-transform duration-100">
          ...
          <div className="mt-4 font-bold">Total: ${animatedTotal.toFixed(2)}</div>
        </div>
