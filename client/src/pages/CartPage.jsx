...
import { addToCart, fetchCart, removeFromCart, emptyCart } from "../features/cart/cartSlice";
...
const CartPage = () => {
  ...
  useEffect(() => {
    ...
    const handleRemove = (productId, quantity) => {
        dispatch(removeFromCart({ userId, productId, quantity })).then(() => dispatch(fetchCart(userId)));
    };
    
    const handleIncrease = (productId) => {
        dispatch(addToCart({ userId, productId, quantity: 1 })).then(() => dispatch(fetchCart(userId)));
    };

    const handleDecrease = (productId) => {
        dispatch(removeFromCart({ userId, productId, quantity: 1})).then(() => dispatch(fetchCart(userId)));
    };
  ...
  return (
    <div>
      ...
      <div className="max-w-3xl mx-auto p-8">
        <h1 className="text-2xl font-bold mb-6 text-violet-700">Your Shopping Cart</h1>

        {cart && cart.items.length > 0 ? 
        (<>
            <div className="space-y-4">
            {cart.items.map((item) => (
                <CartItemCard key={item.productId} item={item} onRemove={handleRemove} onIncrease={handleIncrease} onDecrease={handleDecrease} />
            ))}
            </div>
            <div className="mt-8 text-right">
                <p className="text-xl font-semibold text-gray-700">
                Total: <span className="text-violet-600">${animatedTotal.toFixed(2)}</span>
                </p>
            </div>
        ...
        : 
        (
            <p className="font-semibold text-lg text-gray-600">Your cart is empty.</p>
        )}
